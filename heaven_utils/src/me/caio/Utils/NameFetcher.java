...
public class NameFetcher {
	...
	public String getName(final String uuid) {
		...
		try {
			PreparedStatement stmt = m.mainConnection.prepareStatement("SELECT * FROM uuidfetcher WHERE `uuid`='" + uuid.toString().replace("-", "") + "';");
	...
	public String getUUID(final String nick) {
		uuid = null;

		try {
			PreparedStatement stmt = m.mainConnection.prepareStatement("SELECT * FROM uuidfetcher WHERE `name`='" + nick + "';");
			ResultSet result = stmt.executeQuery();
			if (result.next()) {
	...
	public void updateAPI(Player p) throws SQLException {
