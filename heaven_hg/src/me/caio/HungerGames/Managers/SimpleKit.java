...
public class SimpleKit {
	...
	public static void addKit(Player sender, String s, SimpleKit sk) {
		if (kits.containsKey(s)) {
			sender.sendMessage(ChatColor.GRAY + "O kit '" + s + "' ja existe");
			return;
		}
		kits.put(s, sk);
		sender.sendMessage(ChatColor.GRAY + "O kit '" + s + "' foi criado com sucesso");
	}

	public static void removeKit(Player sender, String s) {
		if (!kits.containsKey(s)) {
			sender.sendMessage(ChatColor.GRAY + "O kit '" + s + "' nao existe");
			return;
		}
		kits.remove(s);
		sender.sendMessage(ChatColor.GRAY + "O kit '" + s + "' foi removido com sucesso");
	}
	...
	public static void applyKit(Player sender, String s, Player target) {
		...
		if (target == null) {
			for (Player p : Bukkit.getOnlinePlayers()) {
				if (p != sender) {
					p.sendMessage(ChatColor.GRAY + "O kit '" + s + "' foi aplicado com sucesso");
				}
			...
			sender.sendMessage(ChatColor.GRAY + "O kit '" + s + "' foi aplicado com sucesso em todos os players");
			return;
		}
		...
		sender.sendMessage(ChatColor.GRAY + "O kit '" + s + "' foi aplicado com sucesso em " + target.getName());
	}
