package me.caio.HungerGames.NMS;

import java.util.ArrayList;

import me.caio.HungerGames.NMS.Packets.PacketPlayOutTitle;
...
public class Title {
	...
	public void send(Player player) {
		...
		PacketPlayOutTitle packetTitle = new PacketPlayOutTitle(PacketPlayOutTitle.EnumTitleAction.TIMES,
				ChatSerializer.a("{text:\"" + ChatColor.translateAlternateColorCodes('&', this.title) + "\",color:" + this.titleColor.name().toLowerCase() + "}"), this.fadeInTime * (this.ticks ? 1 : 20),
		...
		if (this.subtitle != "") {
			packetTitle = new PacketPlayOutTitle(PacketPlayOutTitle.EnumTitleAction.SUBTITLE,
					ChatSerializer.a("{text:\"" + ChatColor.translateAlternateColorCodes('&', this.subtitle) + "\",color:" + this.subtitleColor.name().toLowerCase() + "}"));
			connection.sendPacket(packetTitle);
		}
	}
